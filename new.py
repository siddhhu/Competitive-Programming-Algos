from flask import Flask, render_template, url_for, flash, redirect,request,session,Response,jsonify. #not confirmed
...
app.config['SQLALCHEMY_DATABASE_URI'] ='postgres://kmehjunwelbxem:3a5bd15721c4b183a610f35e1228e8c3cfb1173010a07d43f7d215dde7f03b20@ec2-3-210-23-22.compute-1.amazonaws.com:5432/d33g4onqiumn8h'  #postgres database connection
...
def signin():
    if request.method == 'POST':
        ...
        ProctorUser = ProctorInfo.query.filter_by(proctor_email=session['email'],proctor_password=session['password']).first()            #fetching proctor detail
        try:
      #student a particular proctor is assigned
        ...
        if registeredUser is not None and AnswerUser is None :      #checking condition for student login
            ...
            try:
                return render_template('newfile.html',registeredUser=registeredUser,duration_exam=session['duration'],college_details=college_details,new=new,old=old,now=now,exam_name=session['exam_name'],start=start,end1=end1,ins=session['instructions'])
            except:
        ...
        elif ProctorUser is not None:                                       #login proctor
            return render_template('proctor.html',proctorname=ProctorUser.proctor_name,college_name=college_details.college_university,sections=sections,proctor_email=ProctorUser.proctor_email)
        try:
...
api.add_resource(QuestionSubmit1, '/submit-test')
